...
int main(void) {
    ...
    shm_race_args_t args = {
        .barrier = &barrier,
        .buf = &buf
    };

    pthread_t hole_punch_thread;
    int ret = pthread_create(&hole_punch_thread, NULL, (void*)shm_race_hole_punc, (void*)&args);
    if (ret < 0) {
    ...
    volatile char dummy = *(char *)buf; // Simulated kernel access during hole punch
